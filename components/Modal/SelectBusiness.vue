...
<template>
    <UModal v-model="props.isOpenSelect" prevent-close>
        <UCard :ui="{
            ...
            <template #default>
                <ul class="flex flex-col business_list">
                    <li class="cursor-pointer text-white flex p-4 hover:bg-black-300" v-for="business in myBusinesses"
                        :key="business.id" @click="selectBusiness(business)">
                        {{ business.name }}
                    </li>
                </ul>
            </template>
